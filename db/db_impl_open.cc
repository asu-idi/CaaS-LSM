      ...
      if (!s.ok()) {
        ...
        for (auto cfd : *versions_->GetColumnFamilySet()) {
          cfd->CreateNewMemtable(*cfd->GetLatestMutableCFOptions(),
                                 seq_per_batch_, kMaxSequenceNumber);
        }
      }
    }
  }
...
Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& log_numbers,
  ...
  for (size_t log_it = 0; log_it < log_numbers.size(); ++log_it) {
    ...
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   ...
                   int(immutable_db_options_.wal_recovery_mode));
        ...
        if (batch_changed) {
          ...
          if (new_count > original_count) {
            ROCKS_LOG_FATAL(
                ...
                "Aborting recovery.",
                log_number, int(immutable_db_options_.wal_recovery_mode),
...
Status DB::Open(const Options& options, const std::string& dbname, DB** dbptr) {
  ...
  cf_options.compaction_dispatcher.reset(
      new RemoteCompactionDispatcher(grpc::CreateChannel(
          "127.0.0.1:8010", grpc::InsecureChannelCredentials())));

...
RemoteCompactionDispatcher::Worker::Worker() {
  rep_ = new Rep();
  rep_->env_options = EnvOptions();
  rep_->env_options.use_mmap_writes = false;
  rep_->env_options.use_mmap_reads = true;
  rep_->env = Env::Default();
  g_isCompactionWorkerNode = true;
}
...
std::future<std::string> RemoteCompactionDispatcher::DoCompaction(
    std::string data) {
  compactionservice::CompactionArgs args;
  args.set_input(data);
  compactionservice::CompactionReply reply;
  grpc::ClientContext grpc_context;
  grpc::Status status =
      stub_->ExecuteCompactionTask(&grpc_context, args, &reply);
  std::future<std::string> str_result;
  if (!status.ok()) {
    std::cout << "Remote compaction grpc fail: " << status.error_message()
              << std::endl;
    LocalWorker worker;
    std::string result = worker.DoCompaction(data);
    std::promise<std::string> promise;
    promise.set_value(result);
    str_result = promise.get_future();
  } else {
    std::promise<std::string> promise;
    promise.set_value(reply.result());
    str_result = promise.get_future();
  }
  return str_result;
}
...
std::string RemoteCompactionDispatcher::Worker::DoCompaction(Slice data) {
  ...
  const std::string toSearch = "kNoEntropy";
  const std::string toReplace = "0";
  size_t pos = context.table_factory_options.find(toSearch);
  while (pos != std::string::npos) {
    context.table_factory_options.replace(pos, toSearch.length(), toReplace);
    pos =
        context.table_factory_options.find(toSearch, pos + toReplace.length());
  }
  std::cout << "RemoteCompactionDispatcher::Worker::DoCompaction" << std::endl;
  ...
  if (context.table_factory.empty()) {
    ...
    std::cout << "factory: " << context.table_factory << std::endl;
    std::cout << "option: " << context.table_factory_options << std::endl;
  ...
  auto get_table_reader = [&](uint64_t file_number, TableReader** reader_ptr) {
    ...
    if (find == table_cache.end()) {
      ...
      if (s.IsInvalidArgument() && s.subcode() == Status::kRequireMmap) {
        ...
        assert(!.use_mmap_reads);
        EnvOptions mmap_env_options = env_opt;
        mmap_env_options.use_mmap_reads = true;
        mmap_env_options.use_direct_reads = false;
        mmap_env_options.use_aio_reads = false;

        s = env->NewRandomAccessFile(file_name, &file, mmap_env_options);
        if (!s.ok()) {
          return s;
        }
        file_reader.reset(
  ...
  auto create_builder = [&](std::unique_ptr<WritableFileWriter>* writer_ptr,
    ...
    std::string file_name = cf_options.cf_paths.back().path +
                            GenerateOutputFileName(result.files.size());
    Status s;
  ...
  //   } else {
  //     fprintf(stderr, "ERROR: ReapMatureAction(compaction_filter=%p) =
  //     false\n",
  //             compaction_filter);
  //   }
  // } else {
  ...
  //   fprintf(stderr,
  //           "INFO: EraseFutureAction(secondpass.compaction_filter=%p) =
  ...
  ajson::string_stream stream;
  ajson::save_to(stream, result);
  std::cout << "RemoteCompactionDispatcher::Worker::DoCompaction finish"
            << std::endl;
  return stream.str();
}
